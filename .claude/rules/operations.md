...
4. Copy it to the source: `cp ~/.<file> ~/aloshy-ai/dotfiles/dot_<file>`
...
cd ~/aloshy-ai/dotfiles && git add -A && git commit -m "..." && git push
```
...
source ~/.secrets
docker stop $(docker ps -q --filter "label=devcontainer.local_folder=$HOME/aloshy-ai/devcontainer") 2>/dev/null
devcontainer up --workspace-folder ~/aloshy-ai/devcontainer --remove-existing-container

# Inspect synced files
devcontainer exec --workspace-folder ~/aloshy-ai/devcontainer bash -c "
...
chezmoi source-path   # confirm source dir is ~/aloshy-ai/dotfiles
```
