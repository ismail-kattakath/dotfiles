...
```bash
source ~/.secrets
docker stop $(docker ps -q --filter "label=devcontainer.local_folder=$HOME/aloshy-ai/devcontainer") 2>/dev/null || true
devcontainer up --workspace-folder ~/aloshy-ai/devcontainer --remove-existing-container 2>&1 | tail -10
```

Then inspect what was synced:

```bash
source ~/.secrets && devcontainer exec --workspace-folder ~/aloshy-ai/devcontainer bash -c "
